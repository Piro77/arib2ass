/*****************************************************************************
 * aribb24dec.h : ARIB STD-B24 JIS 8bit character code decoder
 *****************************************************************************
...
static inline int u8_uctomb( unsigned char *s, unsigned int uc, int n )
{
    if( uc < 0x80 && n > 0 )
    {
        s[0] = uc;
...
static int decoder_handle_drcs( arib_decoder_t *decoder, int c )
{
    ...
    {
        drcs_conversion_t *p_drcs_conv =  decoder->p_drcs_conv;
        while( p_drcs_conv != NULL )
        {
            if( strcmp( p_drcs_conv->hash, decoder->drcs_hash_table[c] ) == 0 )
            {
#ifdef DEBUG_ARIBB24DEC
                fprintf( stderr, "drcs hash[%s] converted to U+%x\n",
                         p_drcs_conv->hash,
                         p_drcs_conv->code);
...
static const unsigned int decoder_kanji_table[][94] = {
    ...
    },
    {
        ...
        0x1f14c,0x1f13f,0x1f146,0x1f14b,0x1f221,0x1f21e,0x1f218,0x32ff,
        0x1f142,0x1f213,0x1f21c,0x1f22d,0x1f14d,0x1f131,0x1f13d,0x2b1b,
        0x2b24, 0x1f21d,0x1f214,0x1f228,0x1f22a,0x1f226,0x26bf, 0x1f217,
        0x1f220,0x1f215,0x1f227,0x1f216,0x1f22c,0x1f22b,0x1f22e,0x1f21b,
        0x1f21a,0x1f14e,0x3299, 0x1f200,
    },
    {
...
static void arib_initialize_decoder( arib_decoder_t* decoder, bool b_caption )
{
    decoder->buf = NULL;
    decoder->count = 0;
    decoder->ubuf = NULL;
    ...
    decoder->i_planewidth = 960;
    decoder->i_planeheight = 540;

    decoder->i_width = 620;
    decoder->i_height = 480;
    decoder->i_left = 170;
    decoder->i_top = 30;

    decoder->i_fontwidth = decoder->i_fontwidth_cur
        = decoder->i_fontheight = decoder->i_fontheight_cur = 36;

    decoder->i_horint = decoder->i_horint_cur = 4 /* 4 */;
    decoder->i_verint = decoder->i_verint_cur = 24 /* 24 */;
    ...
    for( int i = 0; i < 10; i++ )
    {
        memset(decoder->drcs_hash_table[i], 0, 32+1);
    }
#endif
...
static void arib_finalize_decoder( arib_decoder_t* decoder )
{
...
static int arib_decode_buffer( arib_decoder_t* decoder,
